...
async function getDets() {
    ...
    const apiKey = 'b0c4b37aad52b2f55f8912d579a47945';
...
if (document.getElementById("heading").classList.contains("title")) {
    document.querySelector(".title").textContent = 'Developerr Bots Files'
}
...
const sourcelink = videolink.replace("/watch/", "/");

function vlc_player() {
    const opensourcelink = sourcelink;
    const openVlc = `vlc://${opensourcelink}`;
    window.location.href = openVlc;
}

function mx_player() {
    const opensourcelink = sourcelink;
    const openMx = `intent:${opensourcelink}#Intent;package=com.mxtech.videoplayer.ad;end`;
    window.location.href = openMx;
}

function playit_player() {
    const opensourcelink = sourcelink;
    const openPlayit = `playit://playerv2/video?url=${opensourcelink}`;
    window.location.href = openPlayit;
}

function s_player() {
    const opensourcelink = sourcelink;
    const openSplayer = `intent:${opensourcelink}#Intent;action=com.young.simple.player.playback_online;package=com.young.simple.player;end`;
    window.location.href = openSplayer;
}

function km_player() {
    const opensourcelink = sourcelink;
    const openKmplayer = `intent:${opensourcelink}#Intent;package=com.kmplayer;end`;
    window.location.href = openKmplayer;
}

function hd_player() {
    const opensourcelink = sourcelink;
    const openHDplayer = `intent:${opensourcelink}#Intent;package=uplayer.video.player;end`;
    window.location.href = openHDplayer;
}

function Download() {
    const opensourcelink = sourcelink;
    window.location.href = opensourcelink;
}